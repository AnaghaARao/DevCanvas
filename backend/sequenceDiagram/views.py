...
from .models import SequenceDiagramNest, SequenceDiagramEntry
...
def generate_sequence_diagram_view(request, doc_id):
    ...
    if not isinstance(file_path, str):
        return Response({'error': 'sequence diagram path is not valid'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
